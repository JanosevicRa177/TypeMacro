...
class RandomSleepCommand(SequencePart):
    ...
    def get_whole_key_sequence(self, function_call: FunctionCall | None) -> list[SequencePart]:
        ...
        if self.min_sleep.parameter_name:
            parameter = function_call.function.get_parameter_by_name(self.min_sleep.parameter_name)
            min_sleep_value = function_call.resolve_attribute_value(parameter.index).get_value()
        max_sleep_value = self.max_sleep.get_value()
        if self.max_sleep.parameter_name:
            parameter = function_call.function.get_parameter_by_name(self.max_sleep.parameter_name)
            max_sleep_value = function_call.resolve_attribute_value(parameter.index).get_value()
        return [MacroCommand(["random_sleep", str(min_sleep_value), str(max_sleep_value)])]
