...
class LoopCommand(SequencePart):
    ...
    def __init__(self, repetitions, sequence: list[SequencePart], parent: Parent):
        ...
        if attribute_type != Type.NUMBER:
            raise Exception("Wrong type in loop command for parameter: " + self.repetitions.get_value())
    ...
    def get_whole_key_sequence(self, function_call: FunctionCall | None) -> list[SequencePart]:
        ...
        if attribute.int_value is None:
            raise Exception("Attribute type missmatch in loop: " + function_call.function.name + " , " + attribute.get_value())
        return flatten_list([sequence_part.get_whole_key_sequence(function_call) for sequence_part in self.sequence]) * attribute.int_value
