...
def interpret_sequence_part(sequence_part, parent) -> SequencePart:
    if sequence_part.if_cursor is not None:
        sequence = [interpret_sequence_part(sequence_part, parent) for sequence_part in sequence_part.if_cursor.if_sequence]
        return IfCursorCommand(sequence, sequence_part.if_cursor.color, parent)
        ...
        if sequence_part.sleep.__class__.__name__ == "RandomSleep":
            return RandomSleepCommand(sequence_part.sleep.min, sequence_part.sleep.max, parent)
    elif sequence_part.loop is not None:
        sequence = [interpret_sequence_part(sequence_part, parent) for sequence_part in sequence_part.loop.sequence]
        return LoopCommand(sequence_part.loop.loop_iterator, sequence, parent)
    elif sequence_part.if_ is not None:
        if_sequence = [interpret_sequence_part(sequence_part, parent) for sequence_part in sequence_part.if_.if_sequence]
        else_sequence = None
        if sequence_part.if_.else_statement is not None:
            else_sequence = [interpret_sequence_part(sequence_part, parent) for sequence_part in sequence_part.if_.else_statement.else_sequence]
        return IfCommand(sequence_part.if_.condition, if_sequence, else_sequence, parent)
...
def interpret_command(command, parent) -> Command:
    if command.auto_pixel_command is not None:
        sequence = [interpret_sequence_part(sequence_part, parent) for sequence_part in command.auto_pixel_command.sequence]
    ...
    if command.macro_command is not None:
        sequence = [interpret_sequence_part(sequence_part, parent) for sequence_part in command.macro_command.sequence]
